...
def calculate_rel_change(row):
    if row["value_ref"] == 0:
        ...
        out = round((row["value_comp"] / row["value_ref"]) - 1, 3)
    return out
...
def calculate_relative_change(df, comp, comparison_reference, on_vars):
    ...
    df_comp = df_comp.rename(columns={
        "scenario_id": "scen_comp", "end_value": "value_comp"})[["scen_comp", "value_comp"] +
                                                                on_vars]
...
def scen_comparison_data(df, max_week, end_method, comparison_reference, model_exclusion=None,
                         calc_week=False, on_vars=None):
    # Model
    if on_vars is None:
    ...
    for scen in df_endvalue["scenario_id"].drop_duplicates():
        ...
        for targ in df_endvalue["target"].drop_duplicates():
            ...
            for model in df_endvalue["model_name"].drop_duplicates():
                if end_method is zeroed_cum_data:
...
def sample_df(df, scenario, pathogen, k=1000):
    ...
    -  For each team_model:
         - create a `"list_sample"`: list of all the possible `"sample_id"` associated with the
           team_model.
           For example: `"modelA_1_scenC"`, `"modelA_2_scenC"`, etc.
         - create a `"`list_weight"` : list of all each weight associated with each `"sample_id"`.
           Calculated as: 1/total number of trajectories of the location, target, scenario,
           model_team.
           For example: 1/100, 1/100, etc.
    - Concatenate all the list for each team_model together.
      For example: `list_sample = "modelA_1_scenC",
      "modelA_2_scenC", …, "modelA_100_scenC", "modelB_1_scenC", "modelB_2_scenC", …,
      modelB_90_scenC", etc.` and `list_weight = 1/100, 1/100, …, 1/100, 1/90, 1/90, …, 1/90, etc.`
    - Transform the list_weight to sum to 1 by dividing by the number of `model_team` and
      `scenario` for the `location`, `target`.
      For example (if we have 2 model team for US, Incident Hospitalization, Flu):
         ...
         `list_weight = list_weight/number of scenario inputted`
         (length of the parameter `scenario`)
    - Sample the `list_sample` k times (by default, 1000) with the associated weight and with
      replacement, by applying:
    ...
    - Select all the individual trajectories data frame input from the complete `"sample"` list.
      If a sample_id is sampled multiple times, it will be repeated multiple times in the output
      data frame.
    - Recode the sample_id to a numeric corresponding to 0 to number of sample - 1
      (by default k-1: 999)
    - Return the output as a data frame with three columns: date, value_<pathogen name>, sample_id

    :parameter df: DataFrame in the SMH standard format containing all the trajectories associated
      with a specific round and pathogen
    :type df: pd.DataFrame
    :parameter scenario: list of scenario to filter the inputted the data frame with. If list is
      empty, an empty DataFrame will be return
    :type scenario: list
    ...
    if len(scenario) > 0:
        ...
        df_scen["sample_id"] = (df_scen["model_name"].astype(str) + "_" +
                                df_scen["type_id"].astype(str) + "_" +
                                df_scen["scenario_id"].astype(str))
        df_scen["pathogen"] = pathogen
        list_sample = weight_sample = []
        ...
        weight_sample_fin = flatten_list(weight_sample) / np.array(
            len(list(df_scen["model_name"].drop_duplicates())))
        ...
        all_sample["sample_id_n"] = list(np.repeat(list(range(k)),
                                                   len(df_scen['horizon'].drop_duplicates())))
        all_sample = all_sample[["value", "target_end_date", "sample_id_n"]]
        all_sample = all_sample.rename(columns={"value": "value" + "_" + pathogen})
    else:
        all_sample = pd.DataFrame(columns=["value" + "_" + pathogen, "target_end_date",
                                           "sample_id_n"])
    return all_sample
...
def prep_multipat_plot_comb(pathogen_information, calc_mean=False):
    ...
    - `"value"`: Sum of all the "value_" columns ("value_<pathogen>" set to NA for pathogen with
      empty DataFrame (not selected))
    - Proportion of each pathogen `"proportion_<pathogen>" = "value_<pathogen>" / "value"`
    - Calculate the median, 95%, 90%, 80%, and 50% quantiles for each "value" and "proportion"
      columns (and the mean if `calc_mean` set to `True`)
    ...
    - `pathogen_information = {<pathogenA>: {"dataframe":<DataFrame>},
      <pathogenB>: {"dataframe":<DataFrame>}, etc.}`
    - `<DataFrame>` is a data frame in the output format of the `sample_df()` function with 3
      columns: "target_end_date", "sample_id_n" and "value_<pathogen>".
    - For more information, please consult the `sample_df()` function documentation

    :parameter pathogen_information: A dictionary containing multiple dictionary containing a
      DataFrame (result of sampling process, key: "dataframe") and named with the associated
      specific pathogen (keys).
    :type pathogen_information: dict
    :parameter calc_mean: Boolean indicating if the mean should be calculated too (in addition to
      the other quantiles)
    :type calc_mean: bool
    :return: A dictionary with 2 objects: (1) "all":  median, 95%, 90%, 80%, and 50% quantiles
      for each "value" and "value_<pathogen>-<quantile>"columns and (2) "detail": median, 95%,
      90%, 80%, and 50% quantiles for each "proportion_<pathogen>-<quantile>" columns.
    """
    ...
    for col in enumerate(f):
        if col[1] not in all_sample.columns:
            all_sample[col[1]] = pd.NA
    # Calculate sum of all pathogen
    all_sample["value"] = all_sample[[col for col in all_sample.columns if
                                      col.startswith('value_')]].sum(axis=1)
    ...
    for patho in pathogen_information:
        ...
        all_sample["proportion_" +
                   pathogen_name] = all_sample["value_" + pathogen_name] / all_sample["value"]
    # Calculate the quantiles for each "value" and "proportion" columns
    all_quantile = all_sample.groupby(["target_end_date"]).agg(f)
    all_quantile.columns = (all_quantile.columns.get_level_values(0) + "-" +
                            all_quantile.columns.get_level_values(1))
