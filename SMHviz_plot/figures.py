...
from SMHviz_plot.utils import make_subplots, make_palette_sequential, subplot_fig_output, \
    subplot_row_col, prep_subplot, color_line_trace, prop_obs_subplot, make_ens_button


def add_scatter_trace(fig, data, legend_name, x_col="time_value", y_col="value", width=2,
                      connect_gaps=None, mode="lines+markers", color="rgb(110, 110, 110)",
                      show_legend=True, subplot_coord=None, hover_text="", line_width=0.0001,
                      visible=True, dash=None, custom_data=None):
    ...
    For more information on the parameters, please consult the plotly.graph_objects.Scatter()
    documentation
    ...
    :parameter subplot_coord: For subplots, a list with 2 values: [row number, column number]
        indicating on which subplots to add the trace. `None` for non subplots object (default)
    ...
    :parameter dash: Option to print the line is dash, options include 'dash', 'dot', and 'dashdot'.
       By default, "None", no dash.
    :type dash: str | None
    ...
    fig.add_trace(go.Scatter(x=data[x_col],
                             ...
                             name=legend_name,
                             mode=mode,
                             marker={"color": color, "line_width": line_width},
...
def add_bar_trace(fig, data, legend_name, x_col="time_value", y_col_max="max", y_col_min="min",
    ...
    for x_date in data[x_col].unique():
        ...
        fig.add_trace(go.Scatter(x=[x_date, x_date],
                                 ...
                                 showlegend=show_leg,
                                 hovertemplate=hover_text + "95% Interval: " +
                                 str(plot_data[y_col_min][0]) + " - " +
                                 str(plot_data[y_col_max][0]) +
                                 "<br>Epiweek: %{x|%Y-%m-%d}<extra></extra>"),
                      row=subplot_coord[0], col=subplot_coord[1])
    fig.update_traces(connectgaps=False)
    return fig


def ui_ribbons(fig, df_plot, quant_sel, legend_name, x_col="target_end_date", y_col="value",
               color=None,
               opacity=0.1, subplot_coord=None, hover_text="", line_width=0.001,
               rm_second_hover=False,
    ...
    :parameter fig: a Figure object to update
    :type fig: plotly.graph_objs.Figure
    :parameter df_plot: a DataFrame containing multiple columns: `type_id`: containing the
        quantiles value associated with the `quant_sel` parameter; `target_end_date`: date (x-axis)
        and `value`: value (y-axis)
    ...
    :parameter y_col: Name of the column to use for y-axis, by default `value`
    :type y_col: str
    :parameter color: Color of the trace to add in a "rgba(X, Y, Z, 1)" format, if `None` (default)
        will use blue: "rgba(0, 0, 255, 1)"
    :type color: str
    ...
    :parameter rm_second_hover: Boolean to remove hover associated with the second `quant_sel`
        value; by default `FALSE`
    :type rm_second_hover: bool
    :parameter show_legend: Boolean to show the legend; by default `False`
    :type show_legend: bool
    :parameter special_hover: If not None, a dictionary contains the hover text for the ribbons
        with the keys: "first" and "second" indication the bottom and the top hover text of the
        ribbon, respectively. If not None will ignore all others "hover" parameters
    ...
    if special_hover is None:
        ...
        if rm_second_hover is False:
            second_hover_text = hover_text + str(round((quant_sel[1] - quant_sel[0]) * 100)) + \
                                (" % Interval: %{customdata:,.2f} - %{y:,.2f}<br>Epiweek:"
                                 "%{x|%Y-%m-%d}<extra></extra>")
        first_hover_text = (hover_text + str(round((quant_sel[1] - quant_sel[0]) * 100)) +
                            " % Interval: %{y:,.2f} - %{customdata:,.2f}<br>Epiweek: "
                            "%{x|%Y-%m-%d}<extra></extra>")
    else:
    ...
    fig.add_trace(go.Scatter(x=df_plot[df_plot["type_id"] == quant_sel[1]][x_col],
                             ...
                             marker={"color": re.sub(', 1\\)', ', ' + str(opacity) + ')',
                                                     color)},
                             legendgroup=legend_name,
                             showlegend=show_legend,
    ...
    fig.add_trace(
        go.Scatter(x=df_plot[df_plot["type_id"] == quant_sel[0]][x_col],
                   ...
                   line={"width": line_width},
                   mode='lines',
                   marker={"color": re.sub(', 1\\)', ', ' + str(opacity) + ')', color)},
                   legendgroup=legend_name,
                   showlegend=False,
...
def make_proj_plot(fig_plot, proj_data, intervals=None, intervals_dict=None,
                   x_col="target_end_date", y_col="value",
                   legend_col="model_name", legend_dict=None, line_width=2,
                   color="rgba(0, 0, 255, 1)",
                   show_legend=True, point_value="median", opacity=0.1, connect_gaps=True,
                   subplot_coord=None,
                   hover_text=""):
    """ Plot projection data on an existing Figure

    Plot projection data on an existing Figure for a specific tasks_id (scenario, target, location,
    model name, etc.)
    ...
    :parameter intervals: List of intervals to plot, by default `None`. If `None` ,it will be set
        to all possible values: `[0.95, 0.9, 0.8, 0.5]`, please use an empty list for no intervals:
        `[]`
    :type intervals: list
    :parameter intervals_dict: Dictionary to translate `intervals` value into associated quantiles
        value, if "None" (default), will use internal dictionary:
            - 0.95: [0.025, 0.975]
            - 0.9: [0.05, 0.95]
            - 0.8: [0.1, 0.9]
            - 0.5: [0.25, 0.75]
    ...
    :parameter y_col: Name of the column to use for y-axis, by default `value`
    :type y_col: str
    :parameter point_value: To plot lines representing "median" (type_id = 0.5) or "point" value
        (type_id = NaN), "None"  for no lines; by default "median".
    :type point_value: str
    :parameter legend_col: Name of the column to use for different traces (one trace per value),
        by default `model_name`
    :type legend_col: str
    :parameter legend_dict: a dictionary with value associated with `legend_col` variable (key, as
        in `proj_data`) and associated full name (value, for  legend purposes). If `None` (default),
        use information from the `legend_col` column for legend
    :type legend_dict: dict
    :parameter line_width: Width of the line, by default `2`
    :type line_width: float | int
    :parameter color: Color of the trace to add in a "rgba(X, Y, Z, 1)" format, (default) will use
        blue: `"rgba(0, 0, 255, 1)"`
    :type color: str
    :parameter show_legend: Boolean to show the legend; by default `True`
    :type show_legend: bool
    :parameter opacity: Opacity level of the intervals (ribbons); by default `None`, will generate
        opacity for each `intervals` value starting at 0.1 with a step of 0.1.
    ...
    if plot_df is not None:
        fig_plot = add_scatter_trace(fig_plot, plot_df, full_model_name, x_col=x_col, y_col=y_col,
                                     ...
                                     show_legend=show_legend, color=color,
                                     subplot_coord=subplot_coord,
    ...
    if intervals is not None:
        if isinstance(intervals, float | int):
            ...
            for i in range(0, len(intervals)):
                ...
                fig_plot = ui_ribbons(fig_plot, df_trace, quant_intervals, full_model_name,
                                      x_col=x_col,
                                      y_col=y_col, color=color, opacity=opacity,
                                      show_legend=ui_show_legend,
                                      subplot_coord=subplot_coord, hover_text=hover_text)
    return fig_plot


def plot_truth_data(fig_plot, truth_facet, truth_data_type, subplot_coord, show_legend,
                    x_truth_col="time_value",
                    y_truth_col="value", truth_legend_name="Truth Data", w_delay=None, line_width=2,
                    connect_gaps=True,
                    truth_mode="lines+markers", color="rgb(200,200,200)"):
    if truth_facet is not None:
        if truth_data_type == "scatter":
            ...
            fig_plot = add_scatter_trace(fig_plot, plot_truth_df, truth_legend_name,
                                         ...
                                         hover_text=truth_legend_name + "<br>",
                                         subplot_coord=subplot_coord,
            ...
            if w_delay is not None:
                ...
                fig_plot = add_scatter_trace(fig_plot, plot_truth_df, truth_legend_name,
                                             show_legend=False,
                                             hover_text=truth_legend_name + "<br>",
                                             subplot_coord=subplot_coord, x_col=x_truth_col,
                                             y_col=y_truth_col,
                                             width=line_width, connect_gaps=connect_gaps,
                                             mode="markers",
                                             color=color, line_width=0.5)
        elif truth_data_type == "bar":
            fig_plot = add_bar_trace(fig_plot, truth_facet, truth_legend_name,
                                     show_legend=show_legend,
                                     hover_text=truth_legend_name + "<br>",
                                     subplot_coord=subplot_coord,
                                     x_col=x_truth_col)
...
def make_scatter_plot(proj_data, truth_data, intervals=None, intervals_dict=None,
                      other_truth_data=None, x_col="target_end_date", y_col="value",
                      point_value="median", legend_col="model_name", x_title="Horizon", y_title="N",
                      subplot_var=None, subplot_title=None, share_x="all", share_y="all",
                      truth_legend_name="Truth Data", legend_dict=None,
                      x_truth_col="time_value",
                      y_truth_col="value", viz_truth_data=True, truth_data_type="scatter",
                      truth_mode="lines+markers",
                      hover_text="", ensemble_name=None, ensemble_color=None, ensemble_view=False,
                      line_width=2,
                      connect_gaps=True, color_dict=None, opacity=0.1, palette="turbo", title="",
                      subtitle="",
                      height=1000, theme="plotly_white", notes=None, button=True, button_opt="all",
                      v_lines=None,
                      h_lines=None, zoom_in_projection=None, specs=None, w_delay=None,
                      other_truth_data_name="Other Truth Data"):
    """Create a Scatter Plot

    Create one plot for model projection output files. The function allows multiple view: adding
    truth data, projection  one or multiple intervals, adding median lines, create subplots per a
    specific variable.
    ...
    :type proj_data: pandas.DataFrame
    :parameter truth_data: Data frame containing the observed data to plot, set to None is no
        observed data plotted
    :type truth_data: pandas.DataFrame
    :parameter intervals: List of intervals to plot, by default `None`. If `None` ,it will be set
        to all possible values: `[0.95, 0.9, 0.8, 0.5]`
    :type intervals: list
    :parameter intervals_dict: Dictionary to translate `intervals` value into associated quantiles
        value, if "None" (default), will use internal dictionary:
            - 0.95: [0.025, 0.975]
    ...
    :type intervals_dict: dict
    :parameter x_col: Name of the column to use for x-axis, by default `target_end_date`
    :parameter other_truth_data: Additional optional Data frame containing the observed data to
        plot, set to None is no additional observed data plotted
    :type other_truth_data: pandas.DataFrame
    :type x_col: str
    :parameter y_col: Name of the column to use for y-axis, by default `value`
    :type y_col: str
    :parameter point_value: To plot lines representing "median" (type_id = 0.5) or "point" value
    ...
    :type y_title: str
    :parameter subplot_var: Name of the column used to create the subplot (for example: subplot by
        list of scenario value associated with the column `scenario_id`)
    ...
    :parameter truth_legend_name: Legend name of the associated trace (used also as legend group
        name), by default "Truth Data"
    :type truth_legend_name: str
    :parameter legend_dict: a dictionary with value associated with `legend_col` variable (key, as
        in `proj_data`) and associated full name (value, for  legend purposes). If `None` (default),
        use information from the `legend_col` column for legend
    :type legend_dict: dict
    :parameter x_truth_col: Name of the `truth_data` column to use for x-axis, by default
        `time_value`
    ...
    :parameter: viz_truth_data: To view (`True`, default) or not (`False`) or in legend only
        (`"legendonly"`) the  truth data
    ...
    :parameter truth_mode: Drawing mode of the truth data, only for scatter trace, by default
        "lines+markers"
    :type truth_mode: str
    :parameter hover_text: Appending text appearing on hover; by default, `""`
    :type hover_text: str
    ...
    :parameter ensemble_view: To view (`True` or not (`False`, default)) only the `ensemble_name`,
        the other trace will be set to "legendonly"
    :type ensemble_view: bool
    :parameter line_width: Width of the line; double for `ensemble_name` (if not `None`), by
        default `2`
    :type line_width: float | int
    :parameter connect_gaps: Boolean to connect the gaps, by default `True`
    :type connect_gaps: bool
    :parameter color_dict: a dictionary with `legend_col` values (key) and associated color in the
        format "rgba(X, Y, Z, 1)" (value), if `None` (default) it will be created by using
        `palette` from plotly.express.colors scale.
    :type color_dict: dict
    :parameter opacity: Opacity level of the intervals (ribbons); by default `0.1`.
    :type opacity: int | float
    :parameter palette: Name of the palette to create `color_dict` if `color_dict` is set to `None`.
        By default, "turbo". Value accepted should come from plotly.express.colors scale.
    :type palette: str
    ...
    :type notes: str
    :parameter button: if `True` (default), add a button on the top right of the plot, called
        "Ensemble" and allowing to display only one trace of interest (`ensemble_name`). Used only
        if `ensemble_name` is also not set to `None`.
    :type button: bool
    :parameter button_opt: if "all" (default), will add an "All" button, displaying all traces.
        Only if `button` is `True` and `ensemble_name` is not `None`
    :type button_opt: str
    :parameter v_lines: Dictionary containing the description of one or possible multiple vertical
        line on the plot, should be structured as:
        `v_lines = {"<name>":{"x": <value>, "line_width": <line width>,
        "line_color": <line color>, "line_dash": <line dash format>}}`,
        by default `None`
    :type v_lines: dict
    :parameter h_lines: Dictionary containing the description of one or possible multiple red
        dashed horizontal line on the plot, should be structured as:
        h_lines = {"<name>":{"value": <value>, "text": <associated text>,
                    "color": <associated color>,
                   "font_color":<associated font color>}, "font_size": <associated font size>}`;
        by default `None`
    :type h_lines: dict
    :parameter zoom_in_projection: Dictionary containing `x_min`, `x_max`, `y_min` and `y_max`
        value, to used as default zoom-in view, by default `None`
    :type zoom_in_projection: dict
    :parameter specs: Parameter `specs` as in the `plotly.subplots.make_subplots()` function. See
      plotly.subplots.make_subplots()` documentation for more details. Used only for plot with
      subplots.
    :type specs: list | None
    :parameter w_delay: For the truth data scatter plot, indicate a ending number of weeks to print
        in mode "markers" only . For example, if set to `4`, the last 4 weeks of the time series
        will be plotted in "markers" mode.
    :type w_delay: int | None
    :parameter other_truth_data_name: Legend name of the associated trace (used also as legend
        group name), by default "Other Truth Data"
    :type other_truth_data_name: str
    ...
    if subplot_var is not None:
        ...
        fig_plot = prep_subplot(sub_var, subplot_title, x_title, y_title, share_y=share_y,
                                share_x=share_x,
    ...
    if intervals_dict is None:
        intervals_dict = {0.95: [0.025, 0.975], 0.9: [0.05, 0.95], 0.8: [0.1, 0.9],
                          0.5: [0.25, 0.75]}
    ...
    if sub_var is not None:
        for var in sub_var:
            ...
            truth_facet = prop_obs_subplot(truth_data, subplot_var, var)
            other_truth_facet = prop_obs_subplot(other_truth_data, subplot_var, var)
            subplot_coord = subplot_row_col(sub_var, var)
            if var == sub_var[0]:
                show_legend = True
            else:
                show_legend = False
            fig_plot = plot_truth_data(fig_plot, truth_facet, truth_data_type, subplot_coord,
                                       show_legend,
                                       x_truth_col=x_truth_col, y_truth_col=y_truth_col,
                                       truth_legend_name=truth_legend_name, w_delay=w_delay,
                                       line_width=line_width,
                                       connect_gaps=connect_gaps, truth_mode=truth_mode)
            fig_plot = plot_truth_data(fig_plot, other_truth_facet, truth_data_type, subplot_coord,
                                       show_legend,
                                       x_truth_col=x_truth_col, y_truth_col=y_truth_col,
                                       truth_legend_name=other_truth_data_name, w_delay=w_delay,
                                       line_width=line_width,
                                       connect_gaps=connect_gaps, truth_mode=truth_mode,
            ...
            for mod_name in list_mod:
                ...
                if (mod_name not in in_legend) and (len(df_facet_trace) > 0):
                    ...
                    show_legend = False
                # Figure add trace
                fig_plot = make_proj_plot(fig_plot, df_facet_trace, intervals=intervals,
                                          intervals_dict=intervals_dict,
                                          x_col=x_col, y_col=y_col, legend_col=legend_col,
                                          legend_dict=legend_dict,
                                          line_width=col_line[1], color=col_line[0],
                                          show_legend=show_legend,
                                          point_value=point_value, opacity=opacity,
    ...
    # Figure without subplots
            ...
            if truth_data_type == "scatter":
                ...
                if w_delay is not None:
                    ...
                    fig_plot = add_scatter_trace(fig_plot, plot_truth_df, truth_legend_name,
                                                 ...
                                                 width=line_width, connect_gaps=connect_gaps,
                                                 mode="markers",
                                                 color="rgb(200, 200, 200)", show_legend=False,
                                                 line_width=0.5)
            elif truth_data_type == "bar":
        ...
        for mod_name in list_mod:
            ...
            col_line = color_line_trace(color_dict, mod_name, ensemble_name=ensemble_name,
                                        ensemble_color=ensemble_color, line_width=line_width)
            # Figure add trace
            fig_plot = make_proj_plot(fig_plot, df_trace, intervals=intervals,
                                      intervals_dict=intervals_dict,
                                      x_col=x_col, y_col=y_col, legend_col=legend_col,
                                      legend_dict=legend_dict,
    ...
    if viz_truth_data is True:
        ...
        if other_truth_data is not None:
            leg_only.append(other_truth_data_name)
    ...
    if button is True and ensemble_name is not None:
        button = make_ens_button(fig_plot, viz_truth_data=viz_truth_data,
                                 truth_legend_name=truth_legend_name,
                                 ensemble_name=ensemble_name, button_name="Ensemble",
                                 button_opt=button_opt)
        fig_plot.update_layout(updatemenus=[{"active": 0, "x": 1.01, "xanchor": 'left',
                                             "type": 'buttons', "buttons": button}])
    # Add vertical lines
    if v_lines is not None:
        for v_name in v_lines:
            v_info = v_lines[v_name]
            fig_plot.add_vline(x=v_info["x"], line_width=v_info["line_width"],
                               line_color=v_info["line_color"],
    ...
    if h_lines is not None:
        for h_name in h_lines:
            ...
            fig_plot.add_hline(y=h_info["value"], line_width=1, line_color=h_info["color"],
                               line_dash="dash",
                               annotation={"font_size": h_info["font_size"],
                                           "font_color": h_info["font_color"],
                                           "font_family": "Arial"},
                               annotation_position="top left",
                               annotation_text=h_info["text"])
    # Update layout
    fig_plot = subplot_fig_output(fig_plot, title=title, subtitle=subtitle, height=height,
                                  theme=theme)
    # Default Zoom-in
    if zoom_in_projection is not None:
        fig_plot.update_xaxes(range=[zoom_in_projection["x_min"], zoom_in_projection["x_max"]],
                              autorange=False)
        fig_plot.update_yaxes(range=[zoom_in_projection["y_min"], zoom_in_projection["y_max"]],
                              autorange=False)
    return fig_plot


def add_point_scatter(fig, df, ens_name, color_dict=None, multiply=1, symbol="circle",
                      ens_symbol="diamond-wide",
                      size=20, opacity=0.7, legend_dict=None, show_legend=True, subplot_col=None,
                      add_zero_line=True,
    ...
    :parameter ens_name: Name of the "Ensemble" variable, to plot on top of the other with a
        specific color (black) and symbol (see parameters `ens_symbol`). The name should be a value
        contains in the column defined via the `legend_col` parameter
    :type ens_name: str
    :parameter color_dict: Dictionary containing each legend value and the associated color, if
        `None` will be created by using the legend_col and palette parameter. Default, `None`
    ...
    :type opacity: int | float
    :parameter legend_dict: a dictionary with value associated with `legend_col` variable (key,
        as in `df`) and associated full name (value, for  legend purposes). If `None` (default),
        use information from the `legend_col` column for legend
    :type legend_dict: dict
    :parameter show_legend: Boolean, to show the legend. By default, True
    :type show_legend: bool
    :parameter subplot_col: Name of the column used to create the subplot (for example: subplot by
        list of scenario value associated with the column `scenario_id`). By default, `None`
        (no subplot)
    :type subplot_col: int | None
    :parameter add_zero_line: Boolean, to add a dot horizontal line showing the 0 axis. By default,
        `True`
    :type add_zero_line: bool
    :parameter legend_col: Name of the column to use for the legend. By default, "model_name"
    :type legend_col: str
    :parameter palette: Name of the palette to apply, if `color_dict` is not set to None.
        By default, `"turbo"`
    :type palette: str
    :return: a plotly.graph_objs.Figure object
    """
    # prerequisite
    ens_marker = {"symbol": ens_symbol, "size": size, "color": 'rgba(0,0,0,' + str(opacity) + ')'}
    multi = multiply
    if fig is None:
        fig = go.Figure()
    ...
    for model in df_comp_model[legend_col].drop_duplicates():
        ...
        model_marker = {"size": 20, "color": color_marker, "symbol": symbol}
...
def make_point_comparison_plot(df, ens_name, plot_comparison=None, title=None, height=1000,
                               theme="plotly_white",
                               color_dict=None, style="individual", x_col="target",
                               x_dictionary=None,
                               x_order=None, subplot=False, subplot_col=None, subplot_titles=None,
                               share_x="all",
                               share_y="all", legend_dict=None, legend_col="model_name",
                               palette="turbo"):
    # Prerequisite
    if style == "inverse":
        multiply = -1
    ...
    if subplot is True:
        ...
        fig = prep_subplot(sub_var, subplot_titles, "", "", sort=False, share_x=share_x,
                           share_y=share_y,
                           row_num=1)
    else:
        fig = go.Figure()
    ...
    if style == "individual":
        ...
        for comparison in plot_comparison:
            ...
            fig = add_point_scatter(fig, df_comp, ens_name, color_dict, legend_dict=legend_dict,
                                    show_legend=show_legend, subplot_col=subplot_col,
                                    add_zero_line=True,
        ...
        for targ in df_all["full_x"].drop_duplicates():
            ...
            for comp in plot_comparison:
                ...
                for comparison in list_comparison:
                    ...
                    fig = add_point_scatter(fig, df_sub_c, ens_name, color_dict,
                                            legend_dict=legend_dict,
                                            show_legend=show_legend, subplot_col=subplot_col,
                                            add_zero_line=True,
                                            legend_col=legend_col, palette=palette,
                                            multiply=multiply)
        fig.update_layout(xaxis1={"tickmode": 'array', "tickvals": tick_x1,
                                  "ticktext": tick_label1})
        fig.update_layout(xaxis2={"tickmode": 'array', "tickvals": tick_x2,
                                  "ticktext": tick_label2})
    # Update Layout
    ...
    if isinstance(plot_comparison, dict):
        legend_param = {"orientation": 'h', "yanchor": 'top', "xanchor": 'left',
                        "traceorder": 'grouped'}
    else:
        legend_param = {"orientation": 'v', "yanchor": 'top', "xanchor": 'left',
                        "traceorder": 'grouped'}
    fig.update_layout(title={"text": title, "font": {'size': 18}, "xanchor": 'center',
                             "xref": 'paper', "x": 0.5, "yref": "paper"}, height=height,
                      template=theme, legend=legend_param)
    return fig


def make_heatmap_plot(df, subplot=False, subplot_col=None, subplot_titles=None, palette="ylorrd",
                      share_x="all", share_y="all", x_col="target_end_date", y_col="location_name",
                      title=None, height=1000, theme="plotly_white", sub_nrow=1, orientation="h"):
    if subplot is True:
        sub_var = list(df[subplot_col].unique())
        fig = prep_subplot(sub_var, subplot_titles, "", "", sort=False,
                           share_x=share_x,  share_y=share_y,  row_num=sub_nrow)
        for var in sub_var:
            df_plot = df[df[subplot_col] == var]
            plot_coord = subplot_row_col(sub_var, var, orientation=orientation)
            fig = fig.add_trace(
                go.Heatmap(z=df_plot["value"], x=df_plot[x_col], y=df_plot[y_col],
                           coloraxis="coloraxis",
                           hovertemplate="x: %{x}<br>y: %{y}<br>z: %{z}<extra></extra>"),
                row=plot_coord[0], col=plot_coord[1])
    else:
        fig = go.Figure(
            data=go.Heatmap(z=df["value"], x=df[x_col], y=df[y_col], coloraxis="coloraxis"))
    fig.update_layout(
        title={"text": title, "font": {'size': 18}, "xanchor": 'center', "xref": 'paper', "x": 0.5,
               "yref": "paper"},
        height=height, template=theme,
        coloraxis={'colorscale': palette, 'colorbar': {'title': "Peak Probability"}})
    return fig
...
def make_boxplot_plot(df, show_legend=False, subplot=False, subplot_col=None, subplot_titles=None,
                      sub_nrow=1,
                      share_x="all", share_y="all", x_col="model_name", y_col="type_id", title=None,
                      box_value=None,
                      height=1000, theme="plotly_white", sub_orientation="v", color_dict=None,
                      box_orientation="h",
    ...
    if subplot is True:
        ...
        for var in sub_var:
            ...
            fig = add_box_plot(df_var, fig, x_col=x_col, y_col=y_col, box_value=box_value,
                               color_dict=color_dict,
                               box_orientation=box_orientation, show_legend=show_legend,
                               plot_coord=plot_coord)
    else:
        fig = go.Figure()
        fig = add_box_plot(df, fig, x_col=x_col, y_col=y_col, box_value=box_value,
                           color_dict=color_dict,
                           box_orientation=box_orientation, show_legend=show_legend,
                           plot_coord=[1, 1])

    fig.update_layout(title={"text": title, "font": {'size': 18}, "xanchor": 'center',
                             "xref": 'paper', "x": 0.5, "yref": "paper"},
                      height=height, template=theme)
    return fig


def add_bar_plot(fig, df_var, df_other=None, truth_data=None, df_x="target_end_date",
                 df_other_x="target_end_date",
                 truth_data_x="time_value", show_legend=True, obs_legend=True, title=None,
                 height=1000, plot_coord=None,
                 var="Pathogen", other_var="Second Pathogen",
                 truth_data_legend_name="Observed Data",
    ...
    if truth_data is not None:
        fig.add_trace(go.Scatter(x=truth_data[truth_data_x], y=truth_data["value"],
                                 ...
                                 marker={"color": 'black'}, visible="legendonly",
                                 showlegend=obs_legend,
                                 hovertemplate=str(
                                     truth_data_legend_name) + ": %{y:,.2f}" + "<extra></extra>"),
                      row=plot_coord[0], col=plot_coord[1])
        if truth_data_tot_col in truth_data.columns:
            fig.add_trace(go.Scatter(x=truth_data[truth_data_x], y=truth_data[truth_data_tot_col],
                                     ...
                                     hovertemplate=str(
                                         truth_data_tot_legend_name) + ": %{y:,.2f}<extra></extra>",
                                     marker={"color": 'lightslategrey'}, visible="legendonly",
                                     showlegend=obs_legend),
                          row=plot_coord[0], col=plot_coord[1])
    fig.add_trace(
        go.Bar(x=df_var[df_x], y=df_var["value"], legendgroup=var, marker={"color": color},
    ...
    if df_other is not None:
        fig.add_trace(go.Bar(x=df_other[df_other_x], y=df_other["value"], legendgroup=other_var,
                             ...
                             marker={"color": color_other},
                             hovertemplate=str(other_var) + ": %{y:,.2f}<extra></extra>"),
                      row=plot_coord[0], col=plot_coord[1])
    fig.update_xaxes(showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.update_yaxes(showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.update_layout(
        title={"text": title, "xanchor": 'center', "xref": 'paper', "x": 0.5, "y": 0.975,
               "yanchor": "middle"},
        height=height, template=theme, barmode="stack", hovermode="x",
        legend={"orientation": 'h', "yanchor": 'top', "xanchor": 'left', "traceorder": 'grouped'}
    )
    return fig


def make_bar_plot(df, df_other=None, truth_data=None, df_x="target_end_date",
                  df_other_x="target_end_date", truth_data_x="time_value", show_legend=True,
                  subplot=False, title=None, height=1000, subplot_col=None,
                  truth_data_legend_name="Observed Data",
                  truth_data_tot_legend_name="Observed Data", df_legend_name="Pathogen",
                  df_other_legend_name="Second Pathogen", subplot_titles=None, share_x="all",
                  share_y="all", x_title="", y_title="N", theme="plotly_white", color_dict=None,
    ...
    if subplot is True:
        ...
        for var in sub_var:
            ...
            fig = add_bar_plot(fig, df_var, df_other=df_other, truth_data=truth_data, df_x=df_x,
                               df_other_x=df_other_x,
                               truth_data_x=truth_data_x, show_legend=show_legend,
                               obs_legend=obs_legend,
                               title=title, height=height, plot_coord=plot_coord, var=var,
                               other_var=other_var,
        ...
        fig = go.Figure()
        fig = add_bar_plot(fig, df, df_other=df_other, truth_data=truth_data, df_x=df_x,
                           df_other_x=df_other_x,
                           truth_data_x=truth_data_x, show_legend=show_legend,
                           obs_legend=show_legend,
...
def add_spaghetti_plot(fig, df, color_dict, legend_dict=None,
                       legend_col="model_name", spag_col="type_id", show_legend=True, hover_text="",
                       opacity=0.3,
    ...
    for leg in df[legend_col].drop_duplicates():
        ...
        # Once Nan's are inserted between typeIDs, insert Nan in type ID col so hover text renders
        # correctly
...
def make_combine_multi_pathogen_plot(list_df, list_pathogen, truth_data=None, opacity=0.2,
                                     color=None, palette="turbo",
                                     intervals_dict=None, intervals=None, bar_interval=0.5,
                                     bar_calc="med", title=None,
                                     y_axis_title="", error_bar_pat=None):
    """Create the Multi-Pathogen Combined plot

    The Multi-pathogen Combined Plot contains 2 subplot representing the combination of multiple
    pathogens trajectories for a specific location, target and a subset of scenario selected by the
    user.

    The first subplot represents the 50%, 80%, 90% and 95% uncertainty intervals for the
    combination of all pathogen and for each pathogen (view updated with a button). It is possible
    to add the observed data (or truth data) on the plot via the `truth_data` parameter.

    The second subplot represents the median proportion of each selected pathogen on a stacked bar
    plot, with a 50% uncertainty intervals (default value) error bars.

    The function `prep_multipat_plot_comb() outputs a dictionary with 2 objects: (1) "all":
    median, 95%, 90%, 80%, and 50% quantiles for each "value" and "value_<pathogen>-<quantile>"
    columns and (2) "detail": median, 95%, 90%, 80%, and 50% quantiles for each
    "proportion_<pathogen>-<quantile>" columns.
    Each quantile is noted as: q1, q2, q3, q4, q5, q6, q7, q8, corresponding to: 0.025, 0.05, 0.1,
    0.25, 0.75, 0.9, 0.95, 0.975, respectively. The median and mean are noted as "med" and "mean",
    respectively.

    :parameter list_df: A dictionary with 2 DataFrame: (1) "all":  median, 95%, 90%, 80%, and 50%
        quantiles for the combined ("value" column) and for each pathogen ("value_<pathogen>"
        columns) and (2) "detail": median, 95%, 90%, 80%, and 50% quantiles proportion for each
        pathogen ("proportion_<pathogen>" columns). Format in the same format as the output of the
        function `prep_multipat_plot_comb()`
    :type list_df: dict
    :parameter list_pathogen: list of pathogen represented in the plot
    :type list_pathogen: list
    :parameter truth_data: A DataFrame containing the observed data with the time in a
        `"time_value"` column, the main pathogen value in a `"value"` column and the combined
        observed value in a `"tot_value"` column. By default, `None`.
    :type truth_data: pd.DataFrame | None
    :parameter opacity: Opacity of the ribbons. By default, 0.2
    :type opacity: float
    :parameter color: A dictionary with the associated color for each pathogen and combined value
        with the name of the pathogen + "combined" as key and the color in "rgba(X,Y,Z,1)" format
        as value. If `"None"`, a dictionary will be created by using the palette parameter by
        default.
    ...
    :parameter intervals_dict: Dictionary to translate `intervals` value into associated quantiles
        value, if "None" (default), will use internal dictionary:
            ...
            - 0.5: [q4, q5]
    :type intervals_dict: dict
    :parameter intervals: List of intervals to plot in the first subplot, by default `None`.
        If `None` , all possible values: `[0.95, 0.9, 0.8, 0.5]` will be plotted
    :type intervals: list
    :parameter bar_interval: Interval to use for the error bar in the second subplot, by default
        `0.5`.
    :type bar_interval: float
    :parameter bar_calc: Value to use for the bar height, should match columns names. By default,
        `"med"`
    :type bar_calc: str
    ...
    :parameter error_bar_pat: Name of the pathogen to draw at the bottom of the plot with error bar
        in the second subplot, by default `None`. If `None`, will take the first pathogen in the `
        list_pathogen` parameters
    ...
    if error_bar_pat is None:
        ...
        print(
            error_bar_pat +
            " is not in `list_pathogen`. The first element of `list_pathogen` will be use instead.")
        error_bar_pat = list_pathogen[0]
    ...
    if intervals_dict is None:
        intervals_dict = {0.95: ["q1", "q8"], 0.9: ["q2", "q7"], 0.8: ["q3", "q6"],
                          0.5: ["q4", "q5"]}
        # Color preparation
        if color is None:
            color = make_palette_sequential(
                pd.DataFrame(data={"pathogen": ["Combined"] + list_pathogen}),
    ...
    for j in ["Combined"] + list_pathogen:
        ...
        for i in range(0, len(intervals)):
            ...
            second_hover_text = (str(round(intervals[i] * 100)) +
                                 " % Interval: %{customdata:,.2f} - %{y:,.2f}<br>"
                                 "Epiweek: %{x|%Y-%m-%d}<extra></extra>")
            first_hover_text = (str(
                round(intervals[i] * 100)) + " % Interval: %{y:,.2f} - %{customdata:,.2f}<br>"
                                             "Epiweek: %{x|%Y-%m-%d}<extra></extra>")
            fig = ui_ribbons(fig, df_plot, quant_sel, y_col="value" + col_name, legend_name=j,
                             color=color[j],
            ...
            fig.for_each_trace(
                lambda trace: trace.update(
                    visible="legendonly") if trace.name != "Combined" else (),
            )
    if truth_data is not None:
        fig = add_scatter_trace(fig, truth_data, list_pathogen[0] + " Observed Data",
                                subplot_coord=[1, 1],
                                hover_text=list_pathogen[0] + "<br>", color="rgba(0,0,0,1)",
                                visible="legendonly")
        if "total_value" in truth_data.columns:
            fig = add_scatter_trace(fig, truth_data,
                                    " + ".join(list_pathogen) + "<br> Observed Data",
    ...
    for pathogen in bar_pathogen_list:
        med_val = df_plot[df_plot["type_id"] == bar_calc][
            "proportion_" + pathogen.lower()].reset_index(drop=True)
        med_point = med_point.subtract(med_val)
        fig.add_trace(go.Bar(
            x=df_plot["target_end_date"], marker={"color": color[pathogen]}, showlegend=False,
            y=med_val, base=med_point, name="bar_" + pathogen), row=2, col=1)
        upper_bar = (
            df_plot[df_plot["type_id"] == quant_sel[1]][
                "proportion_" + pathogen.lower()].reset_index(drop=True))
        lower_bar = (
            df_plot[df_plot["type_id"] == quant_sel[0]][
                "proportion_" + pathogen.lower()].reset_index(drop=True))
        ...
        if pathogen == error_bar_pat:
            fig.update_traces(
                ...
                hovertemplate="Epiweek: %{x|%Y-%m-%d}<br>" + bar_calc.title() + " " + pathogen +
                              ": %{y:,.3f}<br>" + str(
                    bar_interval * 100) + "% Intervals: %{customdata}<extra></extra>",
                error_y={"type": 'data', "symmetric": False, "visible": True,
                         "array": list(upper_bar.subtract(med_val)),
                         "arrayminus": list(med_val.subtract(lower_bar))},
                selector={"name": "bar_" + pathogen})
        else:
            fig.update_traces(
                customdata=med_val,
                hovertemplate="Epiweek: %{x|%Y-%m-%d}<br>" + bar_calc.title() + " " + pathogen +
    ...
    for i in fig.data:
        if i["yaxis"] == "y2":
            ...
            if i["name"] in title_list_pathogen:
                ...
                comb_list.append("legendonly")
                vis_list.append("legendonly")
    button = list([{"label": "Combined", "method": "update",  "args": [{"visible": comb_list}]}])
    button = (button +
              list([{"label": 'Pathogens', "method": 'update', "args": [{'visible': vis_list}]}]))
    # Layout
    fig.update_layout(
        barmode="stack", height=1000,
        legend={"y": 0.5, "yanchor": "bottom", "itemsizing": "constant"},
        updatemenus=[{"active": 0, "x": 1.01, "xanchor": 'left', "type": 'buttons',
                      "buttons": button, "showactive": True}],
        template="plotly_white", yaxis_title=y_axis_title,
        yaxis2_title="Proportion of Each Pathogen"
    )
